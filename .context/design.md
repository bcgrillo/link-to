...
- **Generador** (`build/build.mjs`, Node ≥18, zero-dependencias): lee los `links/*.md`, valida (slug, campos obligatorios, formato de fechas, existencia de imagen), lee las dimensiones reales de la imagen (PNG IHDR / JPEG SOF) y genera en `dist/`:
  - `dist/<slug>/index.html` — página de redirección: `meta refresh` + `location.replace()` + enlace de fallback oculto visualmente (sr-only, accesible); en el `<head>`: `og:*`, `twitter:*`, autor (`meta author` + `article:author`), fechas (`article:published_time`/`modified_time`), tags (`article:tag` + `keywords`), con `og:image` absoluto y dimensiones, `canonical` apuntando al destino, `noindex`.
